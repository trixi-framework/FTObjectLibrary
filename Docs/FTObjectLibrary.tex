...
\section{Introduction}
 ...
 are a few workarounds that exist.
...
% \subsection{Value Classes}
 ...
 Value classes include the base class, FTObject and a subclass, FTValue.
 \begin{itemize}
 \item {\bf FTObject}
 ...
 of this class. Instead you will create subclasses of it that have data and procedures as needed.
 \item {\bf FTValue}
 
  FTValue is a wrapper class that
 allows storage of real, integer, character and logical
 values, which can then be stored in containers. Programmers can create their own value
...
FTDictionary is an ``associative container'', that associates a string to another object. 
...
\item {\bf FTObjectArray}

A mutable one-dimensional array class that can store any object.
...
{\color{blue}\begin{verbatim}
         SUBROUTINE constructDictionary(dict)
                 ...
                 ALLOCATE(v)
                 CALL v % initWithValue(``Ratio of circumference to radius'')
                 obj => v
...
\end{verbatim}}
 ...
 is returned to the calling procedure and can be accessed by the actual argument. The two value objects would normally
...
by way of the release() procedure, leaving only the dictionary to be responsible for deallocating them when it does not need them any more.
 ...
 We use the dictionary a shown in the next snippet of code:
...
{\color{blue}\begin{verbatim}
            ...
            obj => dict % objectForKey("Pi")
            v   => valueFromObject(obj)
            pi = v % realValue()
            
            obj => dict % objectForKey(``definition'')
            v   => valueFromObject(obj)
...
\end{verbatim}}
 ...
 The function valueFromObject() converts the generic object to the specific FTValue type. Finally, the dictionary is
 ...
 containers quite generic, up to the limits of the language.
...
\item You may inherit the ownership of an object by allying a method that creates (allocates and initializes) it.
\item When you no longer need an object you must release it using the release() subroutine, and if you are the last owner, you must deallocate it.
...
\item Converting a generic object to an FTValue
...
\begin{itemize}
		...
		{\color{blue}\begin{verbatim}
	               ...
	               obj => r                            ! r is subclass of FTObject
	               CALL list % insertAfter(obj,record) ! Pointer is retained by list
	               CALL obj % release()                ! If caller reliquishes ownership
		\end{verbatim}}
...
{\color{blue}\begin{verbatim}
              ...
              .
              .
              CALL iterator % setLinkedList(list)
\end{verbatim}}
Or
{\color{blue}\begin{verbatim}
              CLASS(FTLinkedList)        , POINTER :: list
...
However, it can re-size itself if necessary. To be efficient, it adds more than one entries at a time
	...
	\end{verbatim}}
		...
		{\color{blue}\begin{verbatim}
           ...
           IF ( array  %  isUnreferenced() )     THEN
              DEALLOCATE(array) ! If stack is a pointer
              array => NULL()
           END IF 
	\end{verbatim}}
...
The hash table included in the FTObjectLibrary is very simple in that it has a predefined
hash function to describe a sparse matrix with two keys, $(i,j)$. You will initialize the table
with the number of rows.
     {\bf Definition (Subclass of FTObject):}
	{\color{blue}\begin{verbatim}
        	TYPE(FTHashTable) :: table
	\end{verbatim}}
     {\bf Usage:}
...
\begin{itemize}
	...
	{\color{blue}\begin{verbatim}
           TYPE(FTObject) :: obj
           table % addObjectForKeys(obj,i,j)
	\end{verbatim}}
	...
	{\color{blue}\begin{verbatim}
           TYPE(FTObject) :: obj
           obj => table % objectForKeys(i,j)
	\end{verbatim}}
	
        \item Checking if an entry exists
	{\color{blue}\begin{verbatim}
           LOGICAL :: exists
           exists = table % containsKeys()
	\end{verbatim}}
...
\begin{itemize}
      ...
      INTEGER              :: N = 11 ! Should be a power of two.
	...
	\end{verbatim}}
        ...
        \item Converting a base class pointer to an dictionary
	{\color{blue}\begin{verbatim}
