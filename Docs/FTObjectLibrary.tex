...
\item When you no longer need an object (or are going out of scope) you must release it using the release() subroutine.
...
\emph{Fundamentally, if you call an init() method on an object, you should call a release()  in the same scope.}
...
{\color{blue}\begin{verbatim}
      PROGRAM main
         ...
         CALL release(pnt) ! main gives up ownership to pnt
         .
         .
         .
...
\item {\bf destruct()}
      ...
      subroutine on pointers except to call the superclass destruct in a subclasses destructor. For
      non-pointer (stack variables) variables, you call the destruct rather than the release procedure.}
...
For convenience, define an interface to release to be consistent with the library calls:

{\color{blue}\begin{verbatim}
      INTERFACE release  
         MODULE PROCEDURE releaseSubclass
      END INTERFACE
\end{verbatim}}
...
{\color{blue}\begin{verbatim}
            CALL destruct(r)   !For Non-Pointer objects
            CALL release(r)     !For Pointers
\end{verbatim}}
...
\begin{itemize}
	...
	\item Checking to see if a linked list circular or not
		...
		{\color{blue}\begin{verbatim}
		               CALL  release(list) ! If list is a pointer
		               CALL list % destruct() !If it is not
		\end{verbatim}}
\end{itemize}
...
{\color{blue}\begin{verbatim}
               CALL  release(iterator)    ! If a pointer
               CALL iterator % destruct() ! If not
\end{verbatim}}
\end{itemize}
...
\begin{itemize}
	...
	\end{verbatim}}
        \item Destruction
	{\color{blue}\begin{verbatim}
           CALL  release(stack)    ! If stack is a pointer
           call stack % destruct() ! If not
	\end{verbatim}}
...
\begin{itemize}
	...
	\end{verbatim}}
        ...
        \item Destruction
		{\color{blue}\begin{verbatim}
           CALL  release(array) !If array is a pointer
           CALL array  %  destruct() !If not
	\end{verbatim}}
...
\begin{itemize}
	...
	\end{verbatim}}
        ...
        \item Destruction
		{\color{blue}\begin{verbatim}
           CALL  release(matrix)      !If matrix is a pointer
           CALL matrix  %  destruct() !Otherwise
	\end{verbatim}}
...
\begin{itemize}
	...
	\end{verbatim}}
        ...
        \item Destruction
		{\color{blue}\begin{verbatim}
           CALL  release(table)      !If table is a pointer
           CALL table  %  destruct() !Otherwise
	\end{verbatim}}
...
\begin{itemize}
	...
	\end{verbatim}}
        ...
        \item Destruction
		{\color{blue}\begin{verbatim}
           CALL release(dict)       !If dict is a pointer
           CALL dict  %  destruct() !Otherwise
	\end{verbatim}}
