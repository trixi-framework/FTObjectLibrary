...
FTHashTable is an ``associative container'', that associates a double index (i,j) to and FTObject. Basically this is a two dimensional sparse matrix of pointers to FTObjects.
...
initializer that has the most arguments. For example, the designated initializer for a ``point'' class would be the one that takes the (x,y,z) values.
...
{\bf Implementing destruct()}
 ...
 a call to the superclass method. For example, overriding destruct looks like
